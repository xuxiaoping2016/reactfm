...
class CascadeMulti extends React.Component {
    static getDerivedStateFromProps(props, state) {
        ...
        const { dataList, allItemDisabled } = CascadeMulti.setData(
            value,
            options,
            keyCouldDuplicated
        );
    ...
    static setData(value, options, keyCouldDuplicated) {
        ...
        if (dataList && dataList.length) {
            const res = CascadeMulti.setCleanResult(
                dataList,
                true,
                keyCouldDuplicated
            );
        ...
        if (dataList && dataList.length) {
            for (let i = 0, len = dataList.length; i < len; i += 1) {
                ...
                if (dataList[i].children) {
                    const item = CascadeMulti.getTreeNodeData(
                        dataList[i].children,
                        theKey,
                        dataList[i],
                        keyCouldDuplicated
    ...
    static setFatherCheckState(item, checked, dataList, keyCouldDuplicated) {
        ...
        if (parentNode) {
            const halfChecked = CascadeMulti.getBotherCheckedState(
                parentNode.children,
                !checked
            );
            if (halfChecked) {
        ...
        if (botherList && botherList.length) {
            for (let i = 0, len = botherList.length; i < len; i += 1) {
                ...
                if (state) {
                    ...
                    // 要么未选中，要么半选中状态
                    if (
                        (!botherList[i].checked &&
                            !botherList[i].halfChecked) ||
            ...
            function afterClean() {
                if (isCleanDisabledLabel) {
                    ...
                    const {
                        disabledNodes,
                        leafNodes: leafList,
                    } = getDisabledValueLabel(dataList, value);
                    const valueList = [];
                    const labelList = [];
                    ...
                    onSelect(
                        ...
                        labelList,
                        leafList,
                        getCascadeSelected(this.state.dataList, valueList)
                    );
                }
            }
        );
        ...
        if (dataList && dataList.length) {
            dataList.forEach(item => {
                ...
                if (item.halfChecked) {
                    ...
                    backItem.children = this.getSelectResult(
                        item.children,
                        arr,
                        textArr,
                        backItem.children
    ...
    setSelectResult() {
        ...
        const cascadeSelected = this.getSelectResult(
            this.state.dataList,
            arr,
            this.textArr
        );
    ...
    renderUlList(level) {
        ...
        return (
            <div
                key={level}
                className={`${prefixCls}-content ${prefixCls}-content-level${level}`}
            >
                ...
                >
                    {selectArray[level - 1] && !listArray.length ? (
                        <span
                            className={classnames([`${prefixCls}-list-noData`])}
                        >
                            {noDataText}
                        </span>
                    ) : (
    ...
    checkAll() {
        ...
        let [valueList, labelList, leafList] = [[], [], []];
        let current = null;
        let q = options.slice(0);
        while ((current = q.pop())) {
            if (current.children) {
                q.push(...current.children);
            } else {
                leafList.push(current);
            }
        }
        options = options.map(item => {
            valueList.push(item.value);
            labelList.push(item.label);
            return { ...item, halfChecked: false };
        });
        onSelect(valueList, labelList, leafList, options);
    }
    ...
    renderCheckAllItem(notCheckedAll) {
        ...
        return (
            ...
            >
                <label className={`${prefixCls}-item-label`}>
                    <s
                        onClick={() => {
                            notCheckedAll ? this.checkAll() : this.unCheckAll();
                        }}
                        className={classnames({
                            'kuma-tree-checkbox': true,
                            'kuma-tree-checkbox-checked': !notCheckedAll,
                        })}
                    />
                    {notCheckedAll ? '全选' : '全不选'}
                </label>
            </li>
        );
    }
    ...
    renderListItems(dataList, level) {
        const { prefixCls, config, mode, showCheckAll } = this.props;
        const { selectArray } = this.state;
        const arr = [];
        if (level === 0 && showCheckAll) {
            const notCheckedAll = dataList.some(item => !item.checked);
            arr.push(this.renderCheckAllItem(notCheckedAll));
        }
        // 设置当前级是否开启 checkbox
        ...
        dataList.forEach(item => {
            ...
            if (
                showSearchKeywords &&
                item.label &&
                item.label.indexOf(showSearchKeywords) === -1
            ) {
                return;
            }
            const active = selectArray[level] === item.$id;
            arr.push(
                <li
                    key={item.$id}
                    className={classnames({
                        ...
                        [`${prefixCls}-list-item-active`]: active,
                    })}
                    title={item.label}
                    onClick={() => {
                        this.onItemClick(item, level);
                    }}
                >
                    <label
                        className={classnames({
                    ...
                    >
                        {checkable ? (
                            <s
                                className={classnames({
                                    'kuma-tree-checkbox': true,
                                    'kuma-tree-checkbox-indeterminate':
                                        item.halfChecked,
                                    'kuma-tree-checkbox-checked':
                                        item.checked && !item.halfChecked,
                                    'kuma-tree-checkbox-checkbox-disabled':
                                        item.disabled,
                                    'kuma-tree-checkbox-disabled':
                                        item.disabled,
                                })}
                                onClick={() => {
                                    if (!item.disabled) {
     ...
     */
    renderResult() {
        const { prefixCls, resultTitle, allowClear, locale } = this.props;
        return (
            <div
                className={classnames([`${prefixCls}-result`])}
                ref={r => {
                    this.refResultPanel = r;
                }}
            >
                <div className={classnames([`${prefixCls}-result-title`])}>
                    <span className="selected-title">{resultTitle}</span>
                    {/*
    ...
    renderResultNums() {
        ...
        return <span>({this.handleSelectNums})</span>;
    }
     ...
     */
    renderResultTree() {
        const { prefixCls, renderResult } = this.props;
        const { dataList } = this.state;
        const result = renderResult(dataList);
    ...
    renderTreeListNode(dataList, level) {
        ...
        if (dataList && dataList.length) {
            dataList.forEach(item => {
                if (item.checked || item.halfChecked) {
                    ...
                    // 56 = "已选择"文字宽度
                    style.maxWidth -=
                        level < cascadeSize - 1 && item.checked ? 56 : 0;
        ...
        return <ul className={classnames('tree-node-ul')}>{arr}</ul>;
    }
    ...
    renderExpand(item) {
        ...
        if (
            ...
            arr = !item.expand ? (
                <b className="icon kuma-icon-triangle-down">+</b>
            ) : (
                <b className="icon kuma-icon-triangle-right">-</b>
            );
    ...
    renderDescription() {
        ...
        return displayDesc ? <div style={descStyle}>{displayDesc}</div> : null;
    }
    ...
    render() {
        ...
        for (let i = 0; i < cascadeSize; i += 1) {
            ...
            minWidth = 165 * cascadeSize + 2;
        }
        this.setPanelWidth();
        const back = (
            <div
                style={{ width: minWidth + 212 }}
                className="cascade-multi-select-container"
            >
                <div
                    className={classnames(
                        {

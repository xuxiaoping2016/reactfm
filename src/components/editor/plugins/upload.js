...
Uploader.prototype.setup = function() {
    ...
    input.style.cssText =
        'position: absolute; top: 0; right: 0; opacity: 0; outline: 0; filter:alpha(opacity=0);cursor: pointer; height: ' +
        height +
        '; fontSize: ' +
        Math.max(64, height * 5);
    this.form.appendChild(input);
    _positionForm();
...
Uploader.prototype.submit = function() {
    ...
    if (window.FormData && self._files) {
        ...
        xhr.setRequestHeader('ApiClient', 'saas-pc');
        xhr.setRequestHeader('weimob-pid', window.pid);
        xhr.onreadystatechange = function() {
            if (xhr.readyState == 4) {
                if (xhr.status == 200 || xhr.status == 201) {
                    self.settings.success &&
                        self.settings.success(xhr.responseText);
                } else {
                ...
                if (!uploader) {
                    uploader = new Uploader({
                        ...
                        progress: function(
                            ...
                            if (
                                uploadConfig.progressCallback &&
                                isFunction(uploadConfig.progressCallback)
                            ) {
                                uploadConfig.progressCallback.apply(
                                    uploader,
                    ...
                    uploader.success(function(response) {
                        ...
                        if (
                            uploadConfig.formatResult &&
                            isFunction(uploadConfig.formatResult)
                        ) {
                            result = uploadConfig.formatResult.call(
                                null,
                                result
                            );
                        }
                    ...
                    uploader.error(function() {
                        if (
                            uploadConfig.errorCallback &&
                            isFunction(uploadConfig.errorCallback)
                        ) {
                            uploadConfig.errorCallback.apply(
                                uploader,
                                Array.prototype.slice.call(arguments)
                            );
                        } else {

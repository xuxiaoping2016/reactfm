...
class SwipeableViews extends React.Component {
  ...
  handleSwipeMove = event => {
    ...
    if (this.isSwiping === undefined) {
      ...
      const isSwiping = dx > dy && dx > constant.UNCERTAINTY_THRESHOLD; // 横向距离大于竖向距离，且变动距离大于constant.UNCERTAINTY_THRESHOLD
      ...
      // We are likely to be swiping, let's prevent the scroll event.  滑动的时候阻止滚动事件
      if (dx > dy) {
  ...
  handleScroll = event => {
    ...
    // Ignore events bubbling up.(忽略冒泡事件)
  ...
  handleTransitionEnd() {
    ...
    // The rest callback is triggered when swiping. It's just noise(声音; 响声; 噪音; 吵闹声; 噪声; 干扰; 电子干扰讯号; 干扰信息; 垃圾信息).
    // We filter it out.
